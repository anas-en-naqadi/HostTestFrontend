    ...
    <div className="flex flex-col font-sans gap-6 h-full w-full p-4 sm:p-0 sm:pt-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 sm:max-h-[387px] xl:min-h-[450px]">
          <AdminStatCard
            ...
            value={stats?.trainers || fallbackStats.trainers}
            subTitle="Intern"
          />
          <AdminStatCard
            icon={UserCog}
            title="Total Instructors"
            value={stats?.instructors || fallbackStats.instructors}
            subTitle="Instructor"
          />
          <AdminStatCard
            icon={Book}
            title="Total Courses"
            value={stats?.courses || fallbackStats.courses}
            subTitle="Course"
          />
          <AdminStatCard
            icon={LayoutGrid}
            title="Total Categories"
            value={stats?.categories || fallbackStats.categories}
            subTitle="Categorie"
          />
        </div>
        {/* Popular Courses */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-6 gap-6">
        ...
        <div className="lg:col-span-2">
          <ActiveNowCard
            activeInterns={stats?.activeInterns || fallbackStats.activeInterns}
            activeInstructors={stats?.activeInstructors || fallbackStats.activeInstructors}
          />
        </div>
      </div>
    </div>
