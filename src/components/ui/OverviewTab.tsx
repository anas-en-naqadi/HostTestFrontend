...
export default function OverviewTab({
  ...
  const formatDescription = (text: string) => {
    ...
    const previewParagraphs = paragraphs.slice(0, MAX_DESCRIPTION_PARAGRAPHS);

    return (isExpanded ? paragraphs : previewParagraphs).map(
      (paragraph, index) => (
        <div
          key={index}
          className="text-black mb-4 last:mb-0 leading-relaxed prose prose-sm max-w-none"
        >
    ...
    <div className="w-full px-4  lg:p-4 space-y-6 md:space-y-8">
      ...
      {/* <div className="flex flex-col space-y-4 md:space-y-6"> */}
        ...
        <div className="md:col-span-1">
          <h2 className="text-lg md:text-xl font-bold md:sticky md:top-4">Description</h2>
        </div>
