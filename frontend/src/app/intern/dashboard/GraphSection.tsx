...
export default function GraphSection() {
  ...
  if (isError) {
    return (
      ...
      Failed to load dashboard charts. Please try again later.
    </div>
     ...
     tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          // Convert to hours if >= 60, otherwise show minutes
          if (value >= 60) {
            const hours = Math.floor(value / 60);
            const minutes = Math.round(value % 60);
            // Show "1h 30m" format if there are remaining minutes
            return minutes > 0 
              ? `${hours}h ${minutes}m` 
              : `${hours}h`;
          }
          return ` Time spending : ${Math.round(value)}m`;
        }
      }
    } 
    },
