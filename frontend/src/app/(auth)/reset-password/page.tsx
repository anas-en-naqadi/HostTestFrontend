  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAxiosError(err) && err.response?.status === 400) {
        toast.error(
          "Your token has expired or is invalid. You’ll be redirected to resend your reset password link.",
          {
            duration: 2500,
            onAutoClose: () => {
      ...
      >
        ...
        >
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="bg-cyan-800 w-full cursor-pointer flex text-sm md:text-md justify-center items-center gap-2 xl:mt-4 h-12 rounded-md text-white font-semibold"
          >
            {isLoading && (
