...
export default function QuizzesManagement() {
  ...
  const handleEditClick = async (quiz: Quiz) => {
    ...
    try {
      ...
      setSelectedQuiz({
        ...quiz,
        duration: Math.floor(quiz.duration / 60),
  ...
  const columns = useMemo<MRT_ColumnDef<Quiz>[]>(
    () => [
      ...
      },
      {
        ...
        header: "Duration",
        size: 120,
        Cell: ({ cell }) => <span className="flex items-center justify-center  w-full">{formatDuration(cell.getValue<number>())}</span>,
      },
      {
        accessorKey: "questionsCount",
        header: "Number of Questions",
        size: 150,
        Cell: ({ cell }) => <span className="flex items-center justify-center  w-full">{cell.getValue<number>()}</span>,
      },
      {
        accessorKey: "isFinal",
        header: "Is Final",
