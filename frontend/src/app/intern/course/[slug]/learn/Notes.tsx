  ...
  const applyFilters = useCallback((keyword = searchKeyword) => {
    ...
    if (keyword) {
      ...
      result = result.filter(note =>
        ...
        note.module_title?.toLowerCase().includes(lowercaseKeyword) ||
        note.lesson_title?.toLowerCase().includes(lowercaseKeyword)
      );
    }
      ...
      <div className="p-4 flex flex-col w-full gap-12 h-full overflow-x-hidden">
        ...
        {filteredNotes.map((note) => {
          ...
          return (
            ...
            >
              {
                note.noted_at !== 0 && note.lesson_type === "video" ? (
                  <span className="custom-blue-bg text-white font-bold p-1 text-[13px] font-sans rounded-full w-14 text-center h-fit">
                    {formatTime(note.noted_at)}
                  </span>
                ) : (
                    <span
                      className={`
                      text-white font-semibold p-1 text-[13px] font-sans rounded-full w-14 text-center h-fit 
                      ${note.lesson_type === "text"
                                            ? "bg-[#1798DD]" // article
                                            : "bg-[#9F154A]" // quiz
                                          }
                    `}
                    >
                      {note.lesson_type === "text" ? "article" : "quiz"}
                    </span>
                )
              }
                ...
                <div className="flex justify-between items-center">
                  <div className="flex flex-wrap lg:flex-nowrap gap-1 lg:gap-4 items-center">
                    <h1 className="font-extrabold text-sm lg:text-md">
                      {note.module_order_position}.{" "}
                      {note.module_title}
                    </h1>
                    <h2 className="font-medium text-xs lg:text-sm">
                      {note.lesson_order_position}.{" "}
                      {note.lesson_title}
                    </h2>
                  </div>
