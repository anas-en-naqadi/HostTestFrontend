...
import { CourseSaveEvents, emitter } from "@/services/CourseSaveService";
...
export function CourseModal({ open, onClose, course, title }: CourseModalProps) {
    ...
    if (currentLesson.contentType === 'video') {
      ...
      if (currentLesson.video_source === 'upload') {
        ...
        const hasExistingUploadedVideo = editingLessonIndex !== null &&
          currentModule.lessons[editingLessonIndex]?.video_url?.includes('/course_videos');
  ...
  const handleEditLesson = (idx: number) => {
    ...
    if (isNewCourse)
      setCurrentLesson({
        ...lesson,
        video_source: lesson.uploaded_video_file ? 'upload' : 'url',
        isFinalQuiz: lesson.isFinalQuiz // Preserve isFinalQuiz
      });
    else
      setCurrentLesson({
        ...lesson,
        video_source: lesson.video_url?.includes('/course_videos') || lesson.uploaded_video_file ? 'upload' : 'url',
        video_url: lesson.video_url?.includes('/course_videos') || lesson.uploaded_video_file ? '' : lesson.video_url,
        uploaded_video_file: lesson.uploaded_video_file,
        isFinalQuiz: lesson.isFinalQuiz // Preserve isFinalQuiz
      });
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box display="flex" alignItems="center">
            <Typography variant="body2" color="text.secondary" sx={{ mr: 2 }}>
              {formatDuration(getModuleTotalDuration(module) * 60)} | {module.lessons.length} Lessons | Order: {module.order}
            </Typography>
            <IconButton
      ...
      >
        <Box display="flex" alignItems="center">
          <Typography variant="body2" color="text.secondary">
            {formatDuration(lesson.durationMinutes * 60)} • {lesson.contentType} • Order: {lesson.order}
          </Typography>
        </Box>
  ...
  const commitDraft = useCourseDraftStore(state => state.commitDraft);
  const getDraft = useCourseDraftStore(state => state.getDraft);
  ...
  const saveDraftSnapshot = React.useCallback(() => {
    ...
    commitDraft(draft);
    toast.success("Draft stored successfully. Don’t worry!", {
      position: "top-center",
    });

    // Optionnel : console.log('Draft committed to store:', draft);
  }, [getValues, modules, thumbnailFile, introVideoFile, course, commitDraft]);
  ...
  // Load draft or course data when the modal opens
  useEffect(() => {
    if (!open) return;
    ...
    const draft = getDraft(course?.id ?? "new");
    console.log("draft", draft)

    if (draft && (!isNewCourse && draft?.courseId === course?.id) || (draft?.courseId === null && isNewCourse)) {
      toast.info("Draft has been successfully restored", {
        position: "top-center",
      });
      // 1) Charger le brouillon depuis le store
      reset(draft.formValues);
      if (draft.modules && Array.isArray(draft.modules)) {
        const formatted = draft.modules.map((mod: any) => ({
          ...mod,
          lessons: mod.lessons.map((lesson: any) => ({
            ...lesson,
            isFinalQuiz: ensureBoolean(lesson.isFinalQuiz)

          }))
        }));
        setModules(formatted);
      }
      setThumbnailFile(draft.thumbnailFile || null);
      setIntroVideoFile(draft.introVideoFile || null);
      setIntroVideoSource(draft.introVideoSource as "url" | "upload" ?? 'url');
      // Aperçu de la miniature
      if (draft.thumbnailFile) {
        setThumbnailPreview(URL.createObjectURL(draft.thumbnailFile));
      } else if (draft.formValues.thumbnailUrl) {
        setThumbnailPreview(draft.formValues.thumbnailUrl);
      }
      return; // Brouillon trouvé, on arrête ici
    }
    console.log("course", course)

    // 2) Aucun brouillon, charger les données du cours si édition
    if (course) {
      // Détails de base
      reset({
        title: course.title ?? "",
        ...
        difficulty: course.difficulty as "beginner" | "intermediate" | "advanced" | "allLevel",
        categoryId: course.category_id ?? 0,
        isPublished: course.is_published ?? false,
        whatYouWillLearn: course.what_you_will_learn ?? [],
        requirements: course.course_requirements ?? [],
        introVideoUrl: !course.intro_video_url?.includes("/intro_videos") ? course.intro_video_url ?? "" : "",
        thumbnailUrl: course.thumbnail_url ?? "",
      });

      // Miniature
      if (course.thumbnail_url) {
        const url = course.thumbnail_url.startsWith("http")
      ...
      if (course.modules && Array.isArray(course.modules)) {
        const formatted = course.modules.map(mod => ({
          id: mod.id,
          title: mod.title,
          ...
          lessons: mod.lessons.map(lesson => ({
            ...
            isFinalQuiz: ensureBoolean(lesson?.quizzes?.isFinal)
          }))
        }));
        setModules(formatted);
      }
      setIntroVideoSource(course?.intro_video_url?.includes("/intro_videos") ? 'upload' : 'url');

      return;
    }

    // 3) Nouveau cours – valeurs par défaut
    reset({
      title: "",
      subtitle: "",
      description: "",
  ...
  const handleModalClose = () => {
    saveDraftSnapshot();
    // ... puis reset tous les états locaux et fermer le modal
    reset({
      title: "",
      subtitle: "",
      description: "",
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      commitDraft({
        ...
        introVideoSource,
        courseId: course?.id ?? null,
      }, { submit: true });          // ← flag “needsSubmission”
      handleModalClose();            // ferme le modal
    } catch (error: any) {
                ...
                <FormControl component="fieldset" sx={{ mb: 2 }}>
                  ...
                  >
                    ...
                    <FormControlLabel value="upload" control={<Radio />} label={
                      course?.intro_video_url?.includes('/intro_videos') ? (
                        <span>
                          Upload Video,
                ...
                {introVideoSource === "url" ? (
                  <Controller
                    ...
                    render={({ field }) => (
                      <TextField
                        ...
                        variant="outlined"
                        fullWidth
                        placeholder="Enter a youtube video or video URL (.mp4, .webm, .ogg, .mov, .mkv)"
                  ...
                  <>  <div className={`p-4 border-2 border-dashed rounded-md ${videoUrlFileError ? 'border-red-500' : (!introVideoFile ? 'border-gray-300' : 'border-green-500')}`}>
                    <Button
                      ...
                      disabled={isSubmitting}
                    >
                      {introVideoFile || course?.intro_video_url?.includes("intro_videos") ? "Change Video" : "Upload Intro Video"}
                      <input
                        type="file"
                        hidden
                    ...
                    {/* Show existing uploaded video info if editing */}
                    {
                      ...
                      course.intro_video_url.includes('/intro_videos') && !introVideoFile && !getValues("introVideoUrl") && (
                          ...
                          <div className="flex-1 mt-2">
                            ...
                            <FormControl component="fieldset" sx={{ mb: 2 }}>
                              ...
                              >
                                <FormControlLabel value="url" control={<Radio />} label="Provide URL" />
                                <FormControlLabel value="upload" control={<Radio />} label={
                                  editingLessonIndex !== null && currentModule.lessons[editingLessonIndex]?.video_url?.includes('/course_videos') ? (
                                    <span>
                                      Upload Video
                            ...
                            {(!currentLesson.video_source || currentLesson.video_source === "url") ? (
                              ...
                              <div className={`p-4 border-2 border-dashed rounded-md mb-2 ${lessonError && currentLesson.video_source === 'upload' ? 'border-red-500' : (!currentLesson.uploaded_video_file ? 'border-gray-300' : 'border-green-500')}`}>
                                ...
                                {!currentLesson.uploaded_video_file && editingLessonIndex !== null &&
                                  currentModule.lessons[editingLessonIndex]?.video_url?.includes('/course_videos') && !currentLesson.video_url && (
                                    <Typography variant="body2" color="text.secondary" sx={{ mt: 1, mb: 1 }}>
                  ...
                  <div>
                    {modules.map((mod, idx) => (
                      <Box key={idx} sx={{ mb: 1, p: 1, borderBottom: '1px solid #eee' }}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {mod.lessons.length} lessons • {formatDuration(getModuleTotalDuration(mod) * 60)}
                        </Typography>
                      </Box>
                    ))}
                    <Box mt={2}>
                      ...
                      <Typography variant="body1">
                        {formatDuration(modules.reduce((total, mod) => total + getModuleTotalDuration(mod) * 60, 0))}
                      </Typography>
                    </Box>
                  </div>
                ) : (
                ...
                onClick={() => {
                  ...
                  if (activeStep === 0) {
                    ...
                    if (introVideoSource === 'url') {
                      ...
                      const hasExistingUploadedVideo = !isNewCourse && course?.intro_video_url?.includes('/intro_videos');
...
//     introVideoSource,
//     slug:course?.slug!,
//     needsSubmission: true // This will trigger the service
//   };
//   commitDraft(draft, { submit: true });
//   // Simulate an error after a short delay
//   setTimeout(() => {
//     emitter.emit('error', { 
//       key: course?.id?.toString() ?? 'new',
//       message: 'Test error - simulated error for testing'
//     });
//   }, 1000);
// }}>
//   Test Error State
