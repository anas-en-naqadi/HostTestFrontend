...
import { AlarmClock, Shapes, Bell, Trophy } from "lucide-react";
...
// Settings link is common to all users
const settingsLink: NavLink = {
  ...
  // Get user role and convert to lowercase for consistency
  ...
  // Add settings link to all users
  navLinks.push(settingsLink);

  console.log("All available links:", navLinks);
        ...
        <div className="flex flex-col flex-1 group-hover:overflow-y-scroll">
          <div className="mt-[60px]">
            ...
            {firstGroupLinks.map(({ href, label, icon: Icon }) => (
              ...
              >
                ...
                <span
                  className={`${
                    isOpen ? "inline-block ml-2" : "hidden"
          ...
          <div className="mt-auto mb-6">
            ...
            <button
              ...
              disabled={isPending}
              type="button"
              className={`flex items-center transition-all duration-200 ${inactiveClasses} px-4 py-2 w-full text-left md:px-0 ${
                isOpen
                  ? "justify-start gap-3"
                  : "md:justify-center gap-0 md:group-hover:px-4 md:group-hover:justify-start md:group-hover:gap-3"
              } rounded-md mt-3 cursor-pointer`}
