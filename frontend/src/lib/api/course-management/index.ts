...
export const changeCourseStatus = async (courseId: number, isPublished: boolean): Promise<{ success: boolean; message: string }> => {
  const { data } = await axiosClient.patch(`/courses/change-status/${courseId}`, { is_published: isPublished });
  return data;
};
